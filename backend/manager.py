...
class Manager:
    def __init__(self, reqs: List[LiveStreamReq]):
    ...
    async def init(self):
        ...
        self.job = self.scheduler.add_job(self.scan, "interval", seconds=10, max_instances=2)
    ...
    async def scan(self):
        ...
        for req in self.reqs:
            ...
            if stream is not None:
                if self.downloaders[req.role] is None:
                    self.downloaders[req.role] = Downloader(req.role, stream, self.scheduler)
                else:
